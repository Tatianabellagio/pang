...
SCRIPT_DIR=$BASE/scripts/mapping_poolseq
PANG_DIR=$BASE/pangenomes

# better: read samples from samples.txt if it exists
...
for SETDIR in "$PANG_DIR"/set_*; do
  SET_NAME=$(basename "$SETDIR")
    ...
    # choose runtime
  TIME_LIMIT="08:00:00"
  if [[ "$SET_NAME" == set_50_rep* || "$SET_NAME" == set_all_rep* ]]; then
  ...
  for SAMPLE in "${SAMPLES[@]}"; do
    ...
    sed \
      -e "s/TIME_LIMIT_HERE/${TIME_LIMIT}/g" \
